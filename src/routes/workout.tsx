...
function WorkoutScreen() {
  ...
  if (!workout) return <Page title={t("workout")}><p>{t("generate_workout_first")}</p></Page>;
  ...
  const groupKey = { Main: "main", Special: "special", General: "general" } as const;
      ...
      {(["Main", "Special", "General"] as const).map((g) => {
        ...
        return (
          <section key={g} className="space-y-2">
            <SectionTitle>{t(groupKey[g])}</SectionTitle>
            {items.map(({ e, idx }) => (
              <ExerciseCard
                key={idx}
                exercise={e}
